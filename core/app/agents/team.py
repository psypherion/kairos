...
# The user's proxy. It represents you in the conversation.
user_proxy = autogen.UserProxyAgent(
    name="UserProxy",
    human_input_mode="NEVER", # Can be set to "ALWAYS" to require manual approval for each step
    max_consecutive_auto_reply=10,
    is_termination_msg=lambda x: x.get("content", "").rstrip().endswith("TERMINATE"),
    code_execution_config=False, # We will not execute code from the user proxy
    system_message="You are the user's representative. Reply TERMINATE when the task is done."
)
...
groupchat = autogen.GroupChat(
    agents=[user_proxy, manager, researcher, ghostwriter],
    messages=[],
    max_round=15,
...
def register_tools(db_session, user_id):
    ...
    retrieve_context_tool = partial(tools.retrieve_context, db=db_session, user_id=user_id)
    create_note_tool_with_context = partial(tools.create_note_tool, db=db_session, user_id=user_id)

    # Register the tools with the agents that should have access to them.
    user_proxy.register_for_execution(name="retrieve_context")(retrieve_context_tool)
    researcher.register_for_llm(name="retrieve_context", description="Search the user's knowledge base for context on a query.")(retrieve_context_tool)

    user_proxy.register_for_execution(name="create_note_tool")(create_note_tool_with_context)
    manager.register_for_llm(name="create_note_tool", description="Create a new note in the user's database.")(create_note_tool_with_context)
